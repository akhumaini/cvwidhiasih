...
class SaleReportXlsx(models.AbstractModel):
	...
	def _add_quotation_header(self, sheet, data, record, workbook):
		...
		sheet.write('M%s' % row,'Quotation No', quo_header_format)
		sheet.write('P%s' % row, record.name)

		row+=1
		sheet.write('M%s' % row,'Quotation Date', quo_header_format)
		sheet.write('P%s' % row, record.date_order)

		row+=1
		sheet.write('M%s' % row,'Sales Person', quo_header_format)
		...
		sheet.write('P%s' % row, record.incoterm.display_name or '')
		...
		sheet.write('P%s' % row, odoo_format_date(env=record.env, value=record.estimated_delivery_date))
	...
	def _write_line(self, sheet, line, row, seq, workbook):
		...
		sheet.write('O%s' % row, line.product_uom_qty, line_format)
	...
	def generate_xlsx_report(self, workbook, data, records):
		for obj in records:
			...
			SO_STATUS = dict(obj._fields['state'].selection).get(obj.state)
			sheet.merge_range('A1:R2', SO_STATUS.upper(), QUOTATION_TITLE)
