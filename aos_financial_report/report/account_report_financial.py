...
class ReportFinancial(models.AbstractModel):
    ...
    def get_account_lines(self, data):
        ...
        child_reports = self.env['account.financial.report'].search([('id', 'in', childs)], order='sequence, code asc')
        #--------
        #child_reports = self.env['account.financial.report'].browse(account_report_ids)
        res = self._compute_report_balance(child_reports, context=data.get('used_context'))
        if data['enable_filter']:
            comparison_res = self._compute_report_balance(child_reports, context=data.get('comparison_context'))
